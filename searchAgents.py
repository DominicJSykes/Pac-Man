    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                nextState = (nextState,visitedCorners)    
                cost = self.costFn(nextState)
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    while remaining_corners:
        distance, corner = min([(util.manhattanDistance(state,corner), corner) for corner in remaining_corners])
        state = corner
...
def foodHeuristic(state, problem):
    ...
    while foodGrid:
        distance, food = max([(mazeDistance(position,food,problem.startingGameState),food) for food in foodGrid])
        return distance
        position = food
